...
def preprocess_for_pacmap(avg_cohort, features_df, selected_features, selected_dependent_variable):
...
def calculate_pacmap(avg_cohort, cohort_title, features_df, selected_features, selected_dependent_variable):
    # Returns: pacmap_data_points = data points, and death_list = markings

    # Filter and transform df to avg_np
    avg_np = preprocess_for_pacmap(avg_cohort, features_df, selected_features, selected_dependent_variable)
    ...
    death_list = []
    for v_icustay_id in avg_cohort['icustay_id'].tolist():
        if avg_cohort.loc[avg_cohort['icustay_id'] == v_icustay_id, selected_dependent_variable].sum() > 0:
            death_list.append(1)
        else:
            death_list.append(0)
