...
def save_csvs_into_cache(project_path: str, use_case_name=None):
    """
    ...
    for patient_file in os.listdir(directory):
        if isfile(join(directory, patient_file)):  # making sure only files, no other dictionaries inside the SQL folder
            patient_data: dataframe = pd.read_csv(join(directory, patient_file))
            temp_patient: Patient = Patient(patient_file[11:-4], patient_data)      # create a new Patient Obj with this
            patient_counter += 1
...
def load_data_from_cache(project_path, use_case_name, delete_existing_cache):
    ...
    if not os.path.isfile(cache_file_path):
        ...
        save_csvs_into_cache(project_path, use_case_name)
